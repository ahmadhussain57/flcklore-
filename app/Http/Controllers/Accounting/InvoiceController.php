...
class InvoiceController extends Controller
{
     ...
     * إنشاء فاتورة من طلب
     */
    public function createFromOrder(Order $order, InvoiceService $service): RedirectResponse
    {
        try {
            ...
            // إذا كان السبب "موجودة مسبقاً"، نعيد التوجيه للفاتورة
            $existing = Invoice::where('order_id', $order->id)->first();
            if ($existing) {
                return redirect()
                    ->route('accounting.invoices.show', $existing)
