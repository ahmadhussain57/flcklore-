                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">{{ __('المخزون') }}</th>
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        @if($product->hasDiscount())
                                            ...
                                            <span class="text-green-600 dark:text-green-400 font-semibold">{{ number_format($product->sale_price, 2) }}</span>
                                        @else
                                            <span class="text-gray-800 dark:text-gray-200 font-semibold">{{ number_format($product->price, 2) }}</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">
                                        ...
                                        @else
                                            @if($product->stock_quantity > 0)
                                                <span class="text-green-600 dark:text-green-400">{{ $product->stock_quantity }}</span>
                                            @else
                                                <span class="text-red-600 dark:text-red-400">{{ __('نفد') }}</span>
                                            @endif
                                        @endif
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        <div class="flex flex-wrap items-center gap-2">
                                            <a href="{{ route('products.show', $product) }}" class="text-indigo-600 dark:text-indigo-400 hover:underline">{{ __('عرض') }}</a>

                                            @if(in_array($product->status, ['draft', 'rejected', 'pending_edit']))
                                                <a href="{{ route('products.edit', $product) }}" class="text-amber-600 dark:text-amber-400 hover:underline">{{ __('تعديل') }}</a>
                                            @endif

                                            @if(in_array($product->status, ['draft', 'rejected']))
                                                <form action="{{ route('products.submit', $product) }}" method="POST" class="inline">
                                                    @csrf
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400">
                                        {{ __('لا توجد منتجات حتى الآن.') }}
                                        <a href="{{ route('products.create') }}" class="text-indigo-600 dark:text-indigo-400 hover:underline">{{ __('أنشئ منتجك الأول') }}</a>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
