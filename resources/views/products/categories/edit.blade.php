    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                ✏️ {{ __('تعديل التصنيف') }}: {{ $category->name }}
            </h2>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-xl rounded-2xl border border-gray-200 dark:border-gray-700">
                <div class="p-6 lg:p-8">
                    ...
                    <form method="POST" action="{{ route('products.categories.update', $category) }}">
                        ...
                        {{-- الرابط الحالي (للعرض فقط) --}}
                        <div class="mb-5">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                {{ __('الرابط (Slug)') }}
                            </label>
                            <div class="px-4 py-2 bg-gray-100 dark:bg-gray-900/30 rounded-xl text-sm text-gray-600 dark:text-gray-400 font-mono">
                                {{ $category->slug }}
                            </div>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                {{ __('ملاحظة: تغيير الاسم لن يغير الرابط تلقائياً.') }}
                            </p>
                        </div>
                        ...
                        <div class="mb-5">
                            ...
                            <textarea name="description" id="description" rows="4"
                                      class="w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $category->description) }}</textarea>
