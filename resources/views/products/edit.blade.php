    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            @if($product->status === 'pending_edit')
                <div class="mb-4 p-4 bg-yellow-100 dark:bg-yellow-900/30 border-l-4 border-yellow-500 text-yellow-800 dark:text-yellow-200 rounded-lg">
                    ⚠️ {{ __('هذه نسخة تعديل معلقة. سيتم تحديث المنتج المنشور بعد الموافقة.') }}
                </div>
            @endif
            ...
            <div class="bg-white dark:bg-gray-800 shadow-xl rounded-2xl border border-gray-200 dark:border-gray-700">
                <div class="p-6 lg:p-8">
                    ...
                    <form method="POST" action="{{ route('products.update', $product) }}" enctype="multipart/form-data" x-data="{ type: '{{ old('type', $product->type) }}' }">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-5">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <label class="cursor-pointer">
                                    ...
                                    <div class="p-4 border-2 rounded-xl text-center peer-checked:border-indigo-500 peer-checked:bg-indigo-50 dark:peer-checked:bg-indigo-900/30 border-gray-200 dark:border-gray-600 transition">
                        ...
                        <div class="mb-5">
                            ...
                            <textarea name="description" id="description" rows="5"
                                      class="w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $product->description) }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" min="0" name="sale_price" id="sale_price" value="{{ old('sale_price', $product->sale_price) }}"
                                       class="w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <div class="mb-5">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-2 p-4 border border-gray-300 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-900/30">
                                ...
                                @empty
                                    <div class="col-span-full text-center text-gray-500 text-sm">{{ __('لا توجد تصنيفات.') }}</div>
                                @endforelse
                            </div>
                        </div>
                        ...
                        @if($product->media->count())
                            <div class="mb-5">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                                    {{ __('الوسائط الحالية') }}
                                    <span class="text-xs text-gray-400">{{ __('(حدد الملفات لحذفها)') }}</span>
                                </label>
                                    ...
                                    @foreach($product->media as $media)
                                        <label class="relative cursor-pointer group block">
                                            <input type="checkbox" name="delete_media[]" value="{{ $media->id }}"
                                                   class="peer sr-only">
                                            <div class="relative rounded-xl overflow-hidden border-2 border-transparent peer-checked:border-red-500 transition">
                                                @if($media->media_type === 'image')
                                                    <img src="{{ $media->path }}" alt="{{ $media->original_name }}" class="w-full h-28 object-cover">
                                                @else
                                                    <video class="w-full h-28 object-cover" preload="metadata">
                                                        <source src="{{ $media->path }}" type="{{ $media->mime_type }}">
                                                    </video>
                                                @endif
                                                <div class="absolute inset-0 bg-black/0 peer-checked:bg-red-500/50 transition flex items-center justify-center">
                                                    <span class="text-white text-2xl opacity-0 peer-checked:opacity-100 transition">🗑️</span>
                                                </div>
                                            </div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">
                                    {{ __('الملفات المحددة سيتم حذفها من Cloudinary نهائياً عند حفظ التعديلات.') }}
                                </p>
                            </div>
                        @endif
                        ...
                        <div class="mb-5">
                            ...
                            @error('media.*') <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span> @enderror
                            @error('delete_media') <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span> @enderror
                        </div>
